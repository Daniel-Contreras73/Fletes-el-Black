<template>
  <div class="max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold tracking-widest" style="color: #DC2626">NUEVO FLETE</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
      ...
      <div v-if="step === 1">
        <div class="grid grid-cols-2 gap-5">
          ...
          <div>
            <label class="block text-red-500 text-sm font-medium mb-1">Número interior / Departamento (opcional)</label>
          ...
          <div>
            ...
            <input v-model="form.contactName" type="text" :placeholder="fullName" :class="inputClass" />
          </div>
          <div>
            ...
            <select v-model="form.destinationType" :class="inputClass">
              ...
              <option value="foraneo_qroo">Foráneo — Dentro de Quintana Roo</option>
              <option value="foraneo_nacional">Foráneo — Fuera de Quintana Roo</option>
          ...
          <div class="col-span-2">
            ...
            <input v-model="form.references" type="text" placeholder="Ej. Maderería 'Gallardo', edificio naranja de dos pisos" :class="inputClass" />
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-3 gap-5">
          <div class="flex flex-col gap-3">
            ...
            <div class="rounded-xl p-3 bg-red-50 border border-red-200 text-xs text-red-700">
              💡 La IA analizará las fotos para identificar objetos y calcular el precio automáticamente.
            </div>
          </div>
      ...
      <div v-else-if="step === 5">
        ...
        <div v-else>
          ...
          <!-- Objetos que necesitan datos manuales -->
      ...
      <!-- PASO 6: Cotización y confirmación -->
      <div v-else-if="step === 6">
        <div v-if="!confirmed" class="py-4">
          ...
          <div class="rounded-xl border border-red-200 bg-red-50 p-4 mb-5">
            <p class="text-sm font-semibold text-red-700 mb-3">¿Cuándo prefieres pagar?</p>
            <div class="grid grid-cols-2 gap-3">
              <button type="button"
                @click="form.paymentPreference = 'AL_RECOGER'"
                :class="form.paymentPreference === 'AL_RECOGER'
                  ? 'border-red-500 bg-red-600 text-white'
                  : 'border-red-300 text-red-700 hover:bg-red-100'"
                class="border-2 rounded-xl py-3 px-4 text-sm font-semibold transition-colors text-center">
                <span class="text-lg block mb-1">📦</span>
                Al recoger
                <p class="text-xs font-normal mt-0.5 opacity-80">Pagas cuando lleguemos por tu carga</p>
              </button>
              <button type="button"
                @click="form.paymentPreference = 'AL_ENTREGAR'"
                :class="form.paymentPreference === 'AL_ENTREGAR'
                  ? 'border-red-500 bg-red-600 text-white'
                  : 'border-red-300 text-red-700 hover:bg-red-100'"
                class="border-2 rounded-xl py-3 px-4 text-sm font-semibold transition-colors text-center">
                <span class="text-lg block mb-1">🚛</span>
                Al entregar
                <p class="text-xs font-normal mt-0.5 opacity-80">Pagas cuando recibas tu carga</p>
              </button>
            </div>
          </div>

          <p v-if="submitError" class="text-center text-sm text-red-500 mb-3">{{ submitError }}</p>

          <button @click="confirmShipment"
            :disabled="submitting || !form.paymentPreference"
            class="flex items-center justify-center gap-2 w-full py-4 rounded-xl text-white font-bold text-base hover:opacity-90 transition-opacity disabled:opacity-40"
            style="background-color: #DC2626">
            <span v-if="submitting">Publicando flete...</span>
            <span v-else>Confirmar y publicar flete →</span>
          </button>
        </div>
        ...
        <div v-else class="text-center py-10">
          ...
          <h3 class="text-xl font-bold text-gray-800 mb-2">¡Flete publicado!</h3>
          <p class="text-gray-500 mb-6 text-sm">Tu flete ha sido publicado. El equipo de Fletes El Black se pondrá en contacto contigo pronto.</p>
          <RouterLink to="/shipments"
            class="inline-block text-white px-8 py-3 rounded-lg font-semibold hover:opacity-90 transition-opacity"
            style="background-color: #DC2626">
            Ver mis fletes
          </RouterLink>
        </div>
...
const form = reactive({
  origin: '',
  destination: '',
  destinationType: 'local' as 'local' | 'foraneo_qroo' | 'foraneo_nacional' | 'belice',
  floorNumber: '',
  contactName: '',
  references: '',
  images: [] as File[],
  ...
  paymentPreference: '' as '' | 'AL_RECOGER' | 'AL_ENTREGAR',
})
...
const serviceTypeLabel = computed(() => {
  ...
  if (form.destinationType === 'foraneo_qroo') return 'Foráneo — Dentro de Quintana Roo'
  if (form.destinationType === 'foraneo_nacional') return 'Foráneo — Fuera de Quintana Roo'
...
const stepTitle = computed(() => {
  const titles: Record<number, string> = {
    ...
    6: confirmed.value ? '¡Flete publicado!' : '¡Tu cotización está lista! Confirma para publicar el flete',
  }
  return titles[step.value] ?? ''
})

const nextLabel = computed(() => {
  if (step.value === 5 && !analyzing.value && !quoting.value && !analyzeError.value && analyzedItems.value.length > 0) return 'Ver cotización'
  return 'Siguiente'
})

const canProceed = computed(() => {
  if (step.value === 1) return !!(form.origin.trim() && form.destination.trim())
...
const confirmShipment = async () => {
  ...
  submitError.value = ''
  try {
    const items = analyzedItems.value.map((item, i) => {
      const desglos = quoteResult.value!.desglose[i]
      return {
        name: item.nombre,
        category: item.tamano_categoria,
        weightKg: item.peso_kg ?? undefined,
        difficulty: item.dificultad,
        basePrice: desglos?.precio_base ?? 0,
        subtotal: desglos?.subtotal ?? 0,
        aiConfidence: item.confianza,
      }
    })
    await shipmentsStore.create({
      originAddress: form.origin,
      originCity: (form.origin.split(',')[0] ?? form.origin).trim() || form.origin,
      destinationAddress: form.destination,
      destinationCity: (form.destination.split(',')[0] ?? form.destination).trim() || form.destination,
      cargoType: form.destinationType,
      paymentPreference: form.paymentPreference,
      totalPrice: quoteResult.value!.total,
      items,
    })
    confirmed.value = true
  } catch {
    submitError.value = 'Error al publicar el flete, intente de nuevo.'
  } finally {
